...
@export var spawn_interval: float = 1.0 # Tempo em segundos entre cada inimigo

@export_category("Random Events")
@export var event_wave_interval_min: int = 3 # Mínimo de ondas para o próximo evento
@export var event_wave_interval_max: int = 7 # Máximo de ondas para o próximo evento
@export var event_chance: float = 0.75 # Chance de um evento ocorrer quando o intervalo é atingido (0.0 a 1.0)
...
func _ready() -> void:
	...
	# Verifica se as configurações de inimigos estão corretas
	...
	# Instancia e adiciona o jogador à cena
...
func start_next_wave():
	...
	# Checa se é hora de um novo evento aleatório
	if current_wave >= next_event_wave:
		check_for_random_event()
		
	# Calcula quantos inimigos terá esta onda
	if current_wave == 1:
...
# Chamada pelo SpawnTimer para spawnar um único inimigo
func _on_spawn_timer_timeout():
	# Garante que o jogador ainda existe e que ainda há inimigos para spawnar nesta onda
	if not is_instance_valid(player_instance):
		spawn_timer.stop()
		return

	if enemies_spawned_this_wave >= enemies_to_spawn_this_wave:
		spawn_timer.stop() # Todos os inimigos da onda foram spawnados
		return

	# Seleciona um tipo de inimigo e uma posição de spawn
	var enemy_to_instantiate = pick_random_enemy_type()
	
	if not enemy_to_instantiate:
		printerr("Erro ao tentar spawnar: tipo de inimigo inválido.")
		return
...
# Aplica os efeitos de um evento aleatório
func apply_random_event(event_data: Dictionary) -> void:
	...
	match event_data.get("type"):
		...
		"enemy_effect":
			# Aplica o efeito a inimigos já existentes
			for enemy in get_tree().get_nodes_in_group("enemy"):
				apply_enemy_event_effect(enemy, effect)
		"puzzle_level":
			# Lógica futura para puzzles
...
func end_current_event() -> void:
	if current_active_event.is_empty(): return # Nenhum evento ativo para encerrar
	
	print("EVENTO ENCERRADO: ", current_active_event.get("text", "Evento Desconhecido"))
	game_ui.hide_event_message() # Esconde a mensagem na UI
	...
	match current_active_event.get("type"):
		...
		_ :
			pass # Não faz nada para tipos desconhecidos
...
func revert_player_event_effect(player_node: CharacterBody2D, effect_data: Dictionary) -> void:
	...
	if effect_data.has("input_modifier"):
		player_node.set_input_modifier("none") # Retorna ao padrão
	if effect_data.has("move_speed_multiplier"):
...
func get_random_upgrades() -> Array:
	if all_possible_upgrades.is_empty():
		printerr("Aviso: Tentando obter upgrades, mas a lista está vazia.")
		return []
...
func _on_player_died() -> void:
	get_tree().paused = false
	# Se quiser, pode limpar o evento ativo aqui também, caso o player morra com ele ativo
	if not current_active_event.is_empty():
		end_current_event()
