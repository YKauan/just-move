...
func update_card_data(data: Dictionary) -> void:
	...
	if is_instance_valid(title_label):
		...
		printerr("ERRO no UpgradeCard: No 'TitleLabel' nao encontrado")
	# Garante que os Nos foram encontrados
	if is_instance_valid(description_label):
		description_label.text = data.get("text", "Descrição...")
	else:
		printerr("ERRO no UpgradeCard: No 'DescriptionLabel' nao encontrado")
	...
	if is_instance_valid(icon):
		...
		printerr("ERRO no UpgradeCard: No 'Icon' nao encontrado.")
